...
use App\Http\Integrations\Stripe\Requests\Coupons\CreateCoupon;
use App\Http\Integrations\Stripe\Requests\Coupons\DeleteCoupon;
use App\Http\Integrations\Stripe\Requests\Coupons\ListAllCoupons;
use App\Http\Integrations\Stripe\Requests\Coupons\RetrieveCoupon;
use App\Http\Integrations\Stripe\Requests\Coupons\UpdateCoupon;
...
class Coupons extends Resource
{
    ...
    public function listAllCoupons(
        ...
        return $this->connector->send(new ListAllCoupons($createdgt, $createdgte, $createdlt, $createdlte, $endingBefore, $expand0, $expand1, $limit, $startingAfter));
    }
    ...
    public function deleteCoupon(string $coupon): Response
    {
        return $this->connector->send(new DeleteCoupon($coupon));
    }
     ...
     * @param  string  $expand1 Specifies which fields in the response should be expanded.
     */
    public function retrieveCoupon(string $coupon, ?string $expand0, ?string $expand1): Response
    {
        return $this->connector->send(new RetrieveCoupon($coupon, $expand0, $expand1));
    }

    public function updateCoupon(string $coupon): Response
    {
        return $this->connector->send(new UpdateCoupon($coupon));
    }
}
